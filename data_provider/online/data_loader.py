...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        for time_col in ['date', 'datetime', 'timestamp', 'time']:
            ...
            raise ValueError("Time column name not found, make sure date/datetime fields are in the CSV")
        # edd
        ...
        if self.features == 'M' or self.features == 'MS':
            df_raw = df_raw[[self.time_col] + cols]
            cols_data = df_raw.columns[1:]
            df_data = df_raw[cols_data]
        elif self.features == 'S':
            df_raw = df_raw[[self.time_col] + cols + [self.target]]
            df_data = df_raw[[self.target]]
        ...
        df_stamp[self.time_col] = pd.to_datetime(df_stamp.date)
        data_stamp = time_features(df_stamp, self.timeenc, freq=self.freq)
        ...
        # Add debug info after reading data
        # print(f"Debug: df_raw.columns = {list(df_raw.columns)}")
        # print(f"Debug: cols after processing = {cols}")
        # print(f"Debug: cols_data = {list(cols_data)}")
