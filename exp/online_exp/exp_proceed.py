...
from data_provider.online.data_factory import get_dataset, get_dataloader
...
class Exp_Proceed(Exp_Online):
    ...
    #     self.flag_online_learning = True
        
    #     # Special handling for TimesNet parameters
    #     if 'TimesNet' in self.args.model:
    ...
    #     else:
    #         # Original TimeMixer etc. model processing logic
    #         outputs = self.model(*batch)
        
    #     f_dim = -1 if self.args.features == 'MS' else 0
    #     outputs = outputs[:, -self.args.pred_len:, f_dim:]
    #     batch_y = batch[1][:, -self.args.pred_len:, f_dim:].to(self.device)
