...
from data_provider.online.data_factory import data_provider, get_dataset, get_dataloader
...
from utils.metrics import metric, update_metrics, calculate_metrics
import torch
...
class Exp_Online(Exp_Main):
    def __init__(self, args):
        ...
        # Add training history data storage
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        if self.args.leakage or 'val' not in self.online_phases:
            ...
            if self.args.local_rank <= 0:
                ...
                mse = self.online(online_data=vali_data, target_variate=None, phase='val')['mse']
                if self.args.local_rank == 0:
    ...
    def online(self, online_data=None, target_variate=None, phase='test', show_progress=False):
        ...
        print(f"[Debug] Exp Class: {self.__class__.__name__}")
        print(f"[Debug] Phase: {phase}")
        print(f"[Debug] Online Learning Rate: {self.args.online_learning_rate}")
        ...
        # Add lists to collect all predictions and ground truth
        all_preds = []
        all_trues = []
        ...
        for i, (recent_data, current_data) in enumerate(online_loader):
            ...
            with torch.no_grad():
                ...
                # Collect predictions and ground truth for MASE computation
                if isinstance(outputs, (tuple, list)):
                    outputs = outputs[0]
                pred_np = outputs.detach().cpu().numpy()
                true_np = current_data[self.label_position].to(self.device).detach().cpu().numpy()
        ...
        # Record efficiency metrics on first iteration
        if self.efficiency_metrics is None:
            ...
            for _ in range(5):  # Test training time for 5 batches
            ...
            with torch.no_grad():
                for _ in range(100):
                    # Simulate a single batch input
                    batch_x = torch.randn(1, self.args.seq_len, self.args.enc_in).to(self.device)
                    batch_x_mark = torch.randn(1, self.args.seq_len, 4).to(self.device)  # Can get dimensions from args
                    dec_inp = torch.zeros(1, self.args.label_len + self.args.pred_len, self.args.dec_in).to(self.device)
                    batch_y_mark = torch.randn(1, self.args.label_len + self.args.pred_len, 4).to(self.device)
        ...
        # Compute MASE outside main loop
        if len(all_preds) > 0 and len(all_trues) > 0:
            ...
            try:
                ...
                # Ensure historical_data is numpy array
                if historical_data is not None:
                    if hasattr(historical_data, 'cpu'):
                        # If CUDA tensor, move to CPU then convert to numpy
                        historical_data = historical_data.cpu().numpy()
                    elif hasattr(historical_data, 'numpy'):
                        # If CPU tensor, convert to numpy directly
                        historical_data = historical_data.numpy()
            ...
            all_trues_concat = np.concatenate(all_trues, axis=0)
            
            # Compute all metrics
            if historical_data is not None:
                mae, mse, rmse, wape, msmape, mase, rse, corr = metric(all_preds_concat, all_trues_concat, historical_data, seasonality)
            else:
                mae, mse, rmse, wape, msmape, mase, rse, corr = metric(all_preds_concat, all_trues_concat)
        else:
            # If no predictions collected, use original metrics
            rmse = metrics.get('RMSE', 0)
            wape = metrics.get('WAPE', 0)
            msmape = metrics.get('MSMAPE', 0)
            mase = -1
        ...
        if self.args.do_predict:
            ...
            # return mse, mae, rmse, wape, msmape, mase
...
class Exp_AdaptZ(Exp_Online):
    """Causal delayed-feedback baseline used for AdaptZ ablations.

    This class follows the delayed-label online update schedule, but it does
    not implement the official ADAPT-Z z-adapter and feature-gradient predictor
    modules.
    """
