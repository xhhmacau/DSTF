...
import settings
from data_provider.online.data_factory import get_dataset
from exp.online_exp.exp_basic import Exp_Basic
from utils.tools import EarlyStopping, adjust_learning_rate, visual, test_params_flop, load_model_compile
...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self, model=None, framework_class=None):
        ...
        # Then initialize LazyLinear
        def has_lazy_linear(module):
            """Recursively check if module contains LazyLinear"""
            if hasattr(module, '_get_name') and 'LazyLinear' in module._get_name():
                return True
            for child in module.children():
                if has_lazy_linear(child):
                    return True
            return False

        # Initialize LazyLinear before parameter computation
        if has_lazy_linear(model):
            try:
                with torch.no_grad():
                    dummy_x = torch.randn(1, self.args.seq_len, self.args.enc_in).to(self.device)
                    
                    # Check model forward method signature
                    import inspect
                    # For DataParallel-wrapped models, check module attribute
                    actual_model = model.module if hasattr(model, 'module') else model
                    sig = inspect.signature(actual_model.forward)
                    
                    if len(sig.parameters) == 1:
                        # DeformableTST only needs x
                        _ = model(dummy_x)
                    else:
                        # Other models need full parameters
                        dummy_x_mark = torch.randn(1, self.args.seq_len, 4).to(self.device) if self.args.timeenc == 1 else torch.randn(1, self.args.seq_len, 7).to(self.device)
                        dummy_x_dec = torch.randn(1, self.args.label_len + self.args.pred_len, self.args.dec_in).to(self.device)
                        dummy_x_mark_dec = torch.randn(1, self.args.label_len + self.args.pred_len, 4).to(self.device) if self.args.timeenc == 1 else torch.randn(1, self.args.label_len + self.args.pred_len, 7).to(self.device)
                        _ = model(dummy_x, dummy_x_mark, dummy_x_dec, dummy_x_mark_dec)
                    print("LazyLinear parameters initialized successfully")
            except Exception as e:
                print(f"Warning: Could not initialize LazyLinear parameters: {e}")

        # Compute parameters
        actual_model = model.module if hasattr(model, 'module') else model
        model_params = sum([param.nelement() for param in actual_model.parameters()])
        if framework_class is not None:
