...
class Down_Up(up.Adaptation_Up):
    ...
    def assign_adaptation(self, adaptation):
        if adaptation is None:
            ...
            # correct：
            if self.scale is not None and self.scale.dim() == 2:
                self.scale = self.scale.unsqueeze(1)
                self.scale2 = self.scale2.unsqueeze(1)
    ...
    def _merge(self, weight, bias):
        ...
        if weight is not None and self.flag_adapt_weight:
            ...
            try:
                # Unified safe processing logic
                if scale2.dim() == 0:
                    # scale2 is a scalar, multiply directly
                    weight = weight * scale2
                elif weight.dim() >= 2:
                    # Check if scale2 shape is valid
                    if len(scale2.shape) > 0 and scale2.numel() > 0:
                        # Try to match weight input channel dimension
                        target_channels = weight.shape[1] if weight.dim() >= 2 else 1
                        
                        if scale2.shape[-1] == target_channels:
                            # Shape matches, try reshape
                            try:
                                ...
                                # Reshape failed, use mean
                                scale_value = scale2.mean()
                        ...
                        # scale2 invalid, use 1.0
                        weight = weight * 1.0
                else:
                    # Other cases, multiply directly
                    weight = weight * scale2
            except (RuntimeError, IndexError, TypeError):
                # Use safe scalar multiplication on any error
                try:
                    scale_value = scale2.mean() if scale2.numel() > 0 else 1.0
                    weight = weight * scale_value
                except:
                    # Final safety net
                    weight = weight * 1.0
    ...
    def _safe_batch_match(self, tensor, target_batch_size):
        """Safely match batch size"""
        if tensor is None:
            return None
            
        try:
            if target_batch_size <= 0:
                return tensor[:0] if tensor.dim() > 0 else tensor
            ...
            if tensor.shape[0] == target_batch_size:
                ...
                # Need to repeat
                repeat_times = (target_batch_size + tensor.shape[0] - 1) // tensor.shape[0]
                repeated = tensor.repeat(repeat_times, *[1]*(tensor.dim()-1))
                return repeated[:target_batch_size]
        except:
            # Return original tensor or create compatible tensor on any error
            if tensor.dim() > 0:
                return tensor[:min(tensor.shape[0], max(1, target_batch_size))]
            else:
                return tensor

    def _ssf_input(self, x: torch.Tensor):
        if self.scale2 is None:
            return x
        ...
        try:
            ...
            # Safely match batch size
            effective_scale2 = self._safe_batch_match(self.scale2, actual_batch_size)
            
            if effective_scale2 is not None and effective_scale2.numel() > 0:
                x = x.reshape(effective_scale2.shape[0], -1, x.shape[-1])
                result = (x * effective_scale2).view(*batch_size, x.shape[-1])
                return result
            else:
                return x
        except:
    ...
    def _ssf(self, res: torch.Tensor):
        ...
        try:
            ...
            actual_batch_size = res.shape[0]
            
            # Safely match batch size
            effective_scale = self._safe_batch_match(self.scale, actual_batch_size)
            effective_shift = self._safe_batch_match(self.shift, actual_batch_size) if self.shift is not None else None
            
            if effective_scale is not None and effective_scale.numel() > 0:
                res = res.view(effective_scale.shape[0], -1, res.shape[-1])
                if self.bias is not None:
                    if effective_shift is not None:
                        res = res * effective_scale + (effective_shift + self.bias)
                    else:
                        res = res * effective_scale
                else:
                    res = res * effective_scale
...
class Conv1d(Down_Up, ssf.Conv1d):
    ...
    def _ssf_input(self, x: torch.Tensor):
        ...
        try:
            ...
            if effective_scale2 is not None and effective_scale2.numel() > 0:
                x = x.view(effective_scale2.shape[0], -1, *x.shape[-2:]) * effective_scale2.unsqueeze(-1)
    ...
    def _ssf(self, res: torch.Tensor):
        ...
        try:
            ...
            if effective_scale is not None and effective_scale.numel() > 0:
                res = res.view(effective_scale.shape[0], -1, *res.shape[-2:])
                if self.bias is not None:
                    if effective_shift is not None:
                        res = res * effective_scale.unsqueeze(-1) + (effective_shift + self.bias).unsqueeze(-1)
                    else:
                        res = res * effective_scale.unsqueeze(-1)
                else:
                    res = res * effective_scale.unsqueeze(-1)
