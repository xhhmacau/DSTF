...
class SamePadConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, groups=1, gamma=0.9):
        ...
        self.register_buffer('grads', torch.zeros(sum(self.grad_dim)))
        self.register_buffer('f_grads', torch.zeros(sum(self.grad_dim)))
        nh=64
    ...
    def fw_chunks(self):
        ...
        if not hasattr(self, 'q_ema'):
            self.register_buffer('q_ema', torch.zeros_like(q))
        else:
        ...
        if self.trigger == 1:
            ...
            old_w = ww @ idx
            # write memory
            s_att = torch.zeros(att.size(0), device=att.device)
            s_att[idx.squeeze().long()] = v.squeeze()
            W = old_w @ s_att.unsqueeze(0)
            mask = torch.ones(W.size(), device=W.device)
