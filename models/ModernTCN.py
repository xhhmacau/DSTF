...
class ModernTCN(nn.Module):
    def __init__(self,patch_size,patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks, large_size, small_size, dims, dw_dims,
        ...
        # Due to complex padding logic in ModernTCN's forward pass,
        # we need to dynamically compute head_nf instead of static computation at init
        # Use a placeholder here; the actual head will be created on first forward
        self.head_nf = None
        self.head = None
    ...
    def forward(self, x, te=None):
        ...
        # Dynamically create head (only on first forward)
        if self.head is None:
            B, M, D, N = x.shape
            self.head_nf = D * N
            self.head = Flatten_Head(self.individual, self.n_vars, self.head_nf, self.target_window,
                                   head_dropout=self.head_dropout).to(x.device)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.downsample_ratio = configs.downsample_ratio if hasattr(configs, 'downsample_ratio') else configs.down_ratio
